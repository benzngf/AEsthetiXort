...
PF_Err calcOpticFlow(PF_InData* in_data, PF_LayerDef* source, PF_LayerDef* target, SeqData* seqData) {
	...
	for (int i = seqData->numHBlock*seqData->numVBlock - 1; i >= 0; i--) {
		...
		seqData->frame.blocks[i].xDisplace = 0;
		seqData->frame.blocks[i].yDisplace = 0; //initialize
		...
		if (found) {
			seqData->frame.blocks[i].xDisplace = (short)distortX;
			seqData->frame.blocks[i].yDisplace = (short)distortY;
			if (tempBlockWidth > 0) {
				int threshold = minDiff* 40 / tempBlockWidth/tempBlockWidth;
				seqData->frame.blocks[i].thres = (unsigned short)threshold;
			}else{
				seqData->frame.blocks[i].thres = 0;
			}
			found = false;
		}else{
			seqData->frame.blocks[i].xDisplace = (short)0;
			seqData->frame.blocks[i].yDisplace = (short)0;
			seqData->frame.blocks[i].thres = (unsigned short)30598;//(255*3)*40 - 2
		}
	...
	if (tempBlockWidth != seqData->blockWidth) {//scale the output
		for (int i = seqData->numHBlock*seqData->numVBlock - 1; i >= 0; i--) {
			seqData->frame.blocks[i].xDisplace = (short)(seqData->frame.blocks[i].xDisplace*(long)DOWNX);
			seqData->frame.blocks[i].yDisplace = (short)(seqData->frame.blocks[i].yDisplace*(long)DOWNY);
		}
	}
	seqData->frame.ready = true;
	suites.HandleSuite1()->host_dispose_handle(hhh);
	
	return err;
}

PF_Point getListToSort(PF_InData* in_data, SeqData* seqData, int x, int y) {
